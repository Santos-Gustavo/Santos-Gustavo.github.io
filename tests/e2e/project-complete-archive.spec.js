...
async function createProject(page, { projectName, clientName, contractNum }) {
  ...
  await page.locator("#companyName").fill("E2E Complete Archive Company");
  await page
  ...
  await page.locator("#location").fill("Rua Complete Archive 123, Porto");
...
test("project can be completed and archived", async ({ page }) => {
  const timestamp = Date.now();

  const projectName = `E2E Complete Archive Project ${timestamp}`;
  const clientName = `E2E Complete Archive Client ${timestamp}`;
  const contractNum = `COMPLETE-ARCHIVE-${timestamp}`;
  ...
  page.once("dialog", async (dialog) => {
    ...
    await dialog.accept("Conclusão E2E para validar ciclo de vida");
  });
  ...
  await page
    .locator('[data-project-filter="completed"]')
    .filter({ visible: true })
    .click();
  ...
  await page
    .locator('[data-project-filter="active"]')
    .filter({ visible: true })
    .click();
  ...
  page.once("dialog", async (dialog) => {
    ...
    await dialog.accept("Arquivo E2E para validar ciclo de vida");
  });
