...
test("deleting a project removes it from the project list", async ({ page }) => {
  const timestamp = Date.now();

  const projectName = `E2E Delete Project ${timestamp}`;
  const clientName = `E2E Delete Client ${timestamp}`;
  const contractNum = `DELETE-${timestamp}`;
  ...
  await page.locator("#companyName").fill("E2E Delete Company");
  await page
  ...
  await page.locator("#location").fill("Rua Eliminar 123, Porto");
